...
#include "llama.h"
export module LlamaServer.Presampler;
...
inline llama_sampler* build_presampler_chain(
    ...
    llama_sampler* sampler = LlamaServer::sampler_make();
    sampler = LlamaServer::sampler_logit_bias(sampler, model, n_bias, logit_bias);
    sampler = LlamaServer::sampler_dist(sampler, seed);
...
private:
    ...
    void rebuild_presampler(const llama_model* model) {
        ...
        biases.reserve(rewind_biases.size());
